    ...
    pub fn parse(cursor: &mut Cursor<Vec<u8>>) -> Result<Option<Frame>> {
        ...
        match opcode {
            OpCode::Ping | OpCode::Pong if length > 125 => {
                return Err(Error::new(Kind::Protocol,
                                      format!("Rejected WebSocket handshake.Received control frame with length: {}.",
                                              length)));
            }
            OpCode::Close if length > 125 => {
                debug!("Received close frame with payload length exceeding 125. Morphing to protocol close frame.", );
                return Ok(Some(Frame::close(CloseCode::Protocol,
