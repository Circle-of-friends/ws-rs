                       ...
                       where F: Fn(&mut ffi::z_stream) -> Option<Result<()>>
    {
        ...
        loop {
            ...
            let out_slice = unsafe {
                slice::from_raw_parts_mut(output.as_mut_ptr().offset(output_size as isize),
...
impl Compresser {
    pub fn new(window_bits: i8) -> Compresser {
        ...
        unsafe {
            ...
            let result = ffi::deflateInit2_(&mut stream,
                                            9,
                                            ffi::Z_DEFLATED,
    ...
    pub fn compress(&mut self, input: &[u8], output: &mut Vec<u8>) -> Result<()> {
        self.stream_apply(input, output, |stream| unsafe {
            match ffi::deflate(stream, ffi::Z_SYNC_FLUSH) {
                ...
                code => {
                    Some(Err(Error::new(Kind::Protocol,
    ...
    pub fn reset(&mut self) -> Result<()> {
        match unsafe { ffi::deflateReset(&mut self.stream) } {
            ffi::Z_OK => Ok(()),
            code => {
...
impl Decompresser {
    pub fn new(window_bits: i8) -> Decompresser {
        ...
        unsafe {
            let mut stream = mem::zeroed();
            let result = ffi::inflateInit2_(&mut stream,
                                            -window_bits as c_int,
                                            ZLIB_VERSION.as_ptr() as *const c_char,
                                            mem::size_of::<ffi::z_stream>() as c_int);
    ...
    pub fn reset(&mut self) -> Result<()> {
        match unsafe { ffi::inflateReset(&mut self.stream) } {
            ffi::Z_OK => Ok(()),
            code => {
                Err(Error::new(Kind::Protocol,
                               format!("Failed to reset compression context: {}", code)))
...
impl Drop for Decompresser {
    fn drop(&mut self) {
        match unsafe { ffi::inflateEnd(&mut self.stream) } {
...
mod test {
    ...
    fn round_trip() {
        for i in 9..16 {
            ...
            com.compress(&data, &mut compressed)
               .expect("Failed to compress data.");
            
            let mut dec = Decompresser::new(i);
            
            dec.decompress(&compressed, &mut decompressed)
