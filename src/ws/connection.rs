    ...
    where H: Handler
{
    pub fn new(tok: Token,
        ...
        Connection {
            ...
            state: Connecting(Cursor::new(Vec::with_capacity(2048)),
                ...
                match err.kind {
                    ...
                    Kind::Protocol => {
                        ...
                        if let Server = self.endpoint {
                            res.get_mut().clear();
                            if let Err(err) = write!(res.get_mut(),
    ...
    fn write_handshake(&mut self) -> Result<()> {
        ...
        if let Connecting(ref req, ref res) = replace(&mut self.state, Open) {
            trace!("Finished writing handshake response to {}",
                   self.peer_addr());
    ...
    fn read_handshake(&mut self) -> Result<()> {
        ...
        if let Connecting(ref req, ref res) = replace(&mut self.state, Open) {
            trace!("Finished reading handshake response from {}",
                   self.peer_addr());
            ...
            if self.settings.key_strict {
                ...
                if req_key != res_key {
                    return Err(Error::new(Kind::Protocol,
                                          format!("Received incorrect WebSocket Accept key: {} vs {}",
                                                  req_key,
                                                  res_key)));
                }
            }
                ...
                if frame.is_final() {
                    match frame.opcode() {
                        ...
                        OpCode::Text => {
                            ...
                            let msg = Message::text(try!(String::from_utf8(frame.into_data())
                        ...
                        OpCode::Close => {
                            ...
                            if let 2 = try!(data.read(&mut close_code)) {
                                let raw_code: u16 = (close_code[0] as u16) << 8 |
                                    (close_code[1] as u16);
                                trace!("Connection to {} received raw close code: {:?}, {:?}",
                                       self.peer_addr(),
                                       raw_code,
                                       close_code);
                        ...
                        OpCode::Continue => {
                            ...
                            if let Some(first) = self.fragments.pop_front() {
                                let size = self.fragments
                                    .iter()
                                    .fold(first.payload().len() + frame.payload().len(),
                                          |len, frame| len + frame.payload().len());
                                match first.opcode() {
                                    OpCode::Text => {
                                        trace!("Constructing text message from fragments: {:?} -> {:?} -> {:?}",
                                               first,
                                               self.fragments.iter().collect::<Vec<&Frame>>(),
                                               frame);
                                        ...
                                        trace!("Calling handler with constructed message: {:?}",
                                               string);
                                        try!(self.handler.on_message(Message::text(string)));
                                    }
                                    OpCode::Binary => {
                                        trace!("Constructing binary message from fragments: {:?} -> {:?} -> {:?}",
                                               first,
                                               self.fragments.iter().collect::<Vec<&Frame>>(),
                                               frame);
                                        let mut data = Vec::with_capacity(size);
        ...
        if self.socket.is_negotiating() {
            ...
            let res = if self.state.is_connecting() {
                ...
                if let Some(len) = try!(self.socket.try_write_buf(&mut self.out_buffer)) {
                    ...
                    let finished = len == 0 ||
                        self.out_buffer.position() ==
                            self.out_buffer.get_ref().len() as u64;
                    if finished {
                        match self.state {
    ...
    pub fn send_message(&mut self, msg: Message) -> Result<()> {
        ...
        if let Some(frame) = try!(self.handler
            ...
            if frame.payload().len() > self.settings.fragment_size {
                ...
                while let Some(chunk) = chunks.next() {
                    if let Some(_) = chunks.peek() {
                        try!(self.buffer_frame(Frame::message(Vec::from(chunk),
                                                              OpCode::Continue,
                                                              false)));
                    } else {
                        try!(self.buffer_frame(Frame::message(Vec::from(chunk),
