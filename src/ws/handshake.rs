...
impl Handshake {
    ...
    pub fn remote_addr(&self) -> Result<Option<String>> {
        Ok(try!(self.request.client_addr())
            .map(String::from)
            .or_else(|| if let Some(addr) = self.peer_addr {
...
impl fmt::Display for Response {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(f,
               "{}",
               try!(from_utf8(&s).map_err(|err| {
                   error!("Unable to format response as utf8: {:?}",
                          err);
                   fmt::Error
               })))
    }
